...
methods::setMethod("getTableSignature", "DBIConnection", function(.data, conn) {
  ...
  special_cols_signature <- modifyList(
    list(
  ...
  signature <- c(
    head(signature, -3), # Our special columns must be in the last three
    head(modifyList(tail(signature, 3), special_cols_signature), 3)
  )
...
methods::setMethod("getTableSignature", "NULL", function(.data, conn) {
  ...
  signature <- c(
    head(signature, -3), # Our special columns must be in the last three
    head(modifyList(tail(signature, 3), special_cols), 3)
  )
