...
In this example, most (if not all) of the values of the `Forecast_C` column will change with each regular update.
...
Also, in order to keep our data organised, we add a column with the date of the forecast, aptly named `ForecastDate`.
...
addresses2 <- addresses %>%
  ...
  mutate(ValidUntil = as.Date(c(NA, !!name_change_date, NA, !!name_change_date))) %>%
