...
of the pair of functions `delta_export()` and `delta_import()`.

As the names suggest, these functions are used in tandem to export a delta from a table and to load this delta onto
(another) table.

A "delta" is a table containing the changes after a given point in time which can be used for use-cases such as
...
delta_batch_1 <- delta_export(
  ...
  db_table = "mtcars",
  timestamp_from  = "2020-01-01 11:00:00",
  timestamp_until = "2020-01-02 12:00:00"
)
...
delta_batch_2 <- delta_export(
  ...
  db_table = "mtcars",
  timestamp_from  = "2020-01-03 13:00:00",
  timestamp_until = "2020-01-04 14:00:00"
)
```
