...
The results of the benchmark are shown graphically below, where we compare the current development version of SCDB
with the current CRAN version.

Note that the benchmark data for the SQLite backend is halved due to the performance of this backend.
...
# Add note for SQLite backends
benchmarks <- benchmarks |>
  dplyr::mutate("database" = paste0(database, ifelse(grepl("SQLite", database, fixed = TRUE), "*", "")))
...
ggplot2::ggplot(
  ...
  ggplot2::labs(caption = "* IMPORTANT: Benchmark data halved for this backend!")
```
