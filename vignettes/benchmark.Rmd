...
```{r benchmark_data}
# We use the example data from the `basic_principles` vignette:
...
data_1 <- head(example_data, 3)
data_2 <- head(example_data, 5)
data_3 <- head(example_data, 5) |>
...
for (benchmark_id in seq_along(conns)) {
  benchmark_on_conn <- local({
    bquote(scdb_updates(conns[[benchmark_id]], data_on_conns[[benchmark_id]]))
  })
  benchmark_exprs <- append(benchmark_exprs, benchmark_on_conn)
}
names(benchmark_exprs) <- names(conns)

microbenchmark::microbenchmark(list = benchmark_exprs,
                               times = 10)
