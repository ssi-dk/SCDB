...
test_that("Logger: logging to file works", {
  ...
  withr::with_options(
    ...
    {
      logger <- Logger$new(db_tablestring = db_tablestring, warn = FALSE)
...
test_that("Logger: logging to db works", {
  for (conn in get_test_conns()) {
    ...
    logger <- Logger$new(db_tablestring = db_tablestring,
                         ts = ts,
                         log_table_id = db_tablestring,
                         log_conn = conn,
                         warn = FALSE)

    log_table_id <- dplyr::tbl(conn, id(db_tablestring, conn), check_from = FALSE)
...
test_that("Logger: all logging simultanously works", {
  for (conn in get_test_conns()) {
    ...
    logger <- Logger$new(db_tablestring = db_tablestring, ts = ts, log_path = log_path,
                         log_table_id = db_tablestring, log_conn = conn, warn = FALSE)
