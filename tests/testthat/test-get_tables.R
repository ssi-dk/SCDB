...
test_that("get_tables() matches the pattern of SCDB::id", {
  for (conn in get_test_conns()) {
    ...
    permanent_table <- id("__mtcars", conn = conn)
    ...
    expect_identical(nrow(get_tables(conn, pattern = paste0("^", as.character(permanent_table)))), 1)
    expect_identical(id(get_tables(conn, pattern = paste0("^", as.character(permanent_table)))), permanent_table)

    expect_identical(nrow(get_tables(conn, pattern = paste0("^", as.character(temporary_table)))), 1)
