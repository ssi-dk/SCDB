...
test_that("as.character.id() works with implicit output", {
  ...
  expect_identical(
    as.character(DBI::Id(catalog = "catalog", schema = "schema", table = "table")),
    "catalog.schema.table"
  )

  expect_identical(as.character(DBI::Id(table = "table", schema = "schema")), "schema.table")
  expect_identical(
    as.character(DBI::Id(table = "table", schema = "schema", catalog = "catalog")),
    "catalog.schema.table"
  )
})
...
test_that("as.character.id() works with explicit output", {
  ...
  expect_identical(
    as.character(DBI::Id(catalog = "catalog", schema = "schema", table = "table"), explicit = TRUE),
    "\"catalog\".\"schema\".\"table\""
  )

  expect_identical(as.character(DBI::Id(table = "table", schema = "schema"), explicit = TRUE), "\"schema\".\"table\"")
  expect_identical(
    as.character(DBI::Id(table = "table", schema = "schema", catalog = "catalog"), explicit = TRUE),
    "\"catalog\".\"schema\".\"table\""
  )
})
